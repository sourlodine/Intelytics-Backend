...
async function fetchDataAndStore() {
    try {
        tokenList.forEach(async (tokens) => {
            ...
            const info = await Token.findOne({ token: tokenData.pairs[0].baseToken.symbol })
            if (info) {
                info.price.push(tokenData.pairs[0].priceUsd)
                info.liquidity.push(tokenData.pairs[0].liquidity.usd)
                info.volume.push(tokenData.pairs[0].volume)
                info.timestamp.push(new Date().toISOString())
                info.save()
            } else {
